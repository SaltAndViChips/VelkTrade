...
function normalizeUser(user) {
  ...
  const developer = Boolean(user?.isDeveloper || user?.is_developer || role === 'developer' || DEVELOPER_NAMES.has(lower(username)));
  const admin = Boolean(developer || user?.isAdmin || user?.is_admin || role === 'admin' || role === 'moderator');
  const verified = Boolean(user?.isVerified || user?.is_verified || user?.isTrusted || user?.is_trusted || role === 'verified' || role === 'trusted');

  let status = lower(user?.status || user?.presence || user?.activity || user?.currentView || user?.view || user?.location || 'online');
...
export default function UnifiedPlayerMenu({ currentUser, onlineUsers = [], currentRoomId = '', onInvitePlayer = () => {} }) {
  ...
  async function loadOnlineUsers() {
    try {
      const data = await tryApi([() => api('/api/online-users'), () => api('/api/users/online')]);
  ...
  async function loadRoleUsers() {
    try {
      const data = await tryApi([() => api('/api/admin/users'), () => api('/api/admin/users/list')]);
  ...
  async function loadNotifications() {
    try {
      const data = await tryApi([() => api('/api/notifications'), () => api('/api/me/notifications')]);
  ...
  useEffect(() => {
    ...
    loadNotifications();
    loadPreferences();
    const onlineTimer = window.setInterval(() => { loadOnlineUsers(); loadRoleUsers(); }, 8000);
  ...
  const users = useMemo(() => {
    ...
    for (const user of fetchedUsers) byKey.set(getUserKey(user), normalizeUser(user));
    ...
    for (const user of roleUsers) {
      ...
      if (byKey.has(key)) byKey.set(key, normalizeUser({ ...byKey.get(key), ...user }));
    }

    return [...byKey.values()]
      .filter(user => user.username)
      .filter(user => Number(user.id) !== Number(currentUser?.id))
      .sort((a, b) => roleRank(b) - roleRank(a) || String(a.username).localeCompare(String(b.username)));
  }, [fetchedUsers, onlineUsers, roleUsers, currentUser]);

  const unreadCount = useMemo(() => notifications.filter(notification => !notification.read && !notification.seen && !notification.isRead).length, [notifications]);

  function openProfile(username) {
    const cleanUsername = String(username || '').trim();
    if (!cleanUsername) return;
    setOpen(false);
    window.location.assign(profileUrl(cleanUsername));
  }
  ...
  async function markAllRead() {
    try {
      await tryApi([() => api('/api/notifications/read-all', { method: 'POST' }), () => api('/api/notifications/mark-all-read', { method: 'POST' })]);
    } catch {}
  ...
  async function acceptRoomInvite(notification) {
    ...
    if (!roomId) return;
    try {
      await tryApi([() => api(`/api/rooms/${roomId}/invite/accept`, { method: 'POST' }), () => api(`/api/rooms/${roomId}/join`, { method: 'POST' })]);
    } catch {}
    window.location.assign(`${window.location.origin}${import.meta.env.BASE_URL || '/'}?room=${encodeURIComponent(roomId)}`);
  }
  ...
  function checkTrade(notification) {
    ...
    if (!tradeId) return;
    window.location.assign(`${window.location.origin}${import.meta.env.BASE_URL || '/'}?trade=${encodeURIComponent(tradeId)}`);
  }

  return (
    <div className="unified-player-menu">
      <button type="button" className="unified-player-trigger" title="Online players and notifications" aria-label="Online players and notifications" onClick={() => setOpen(value => !value)}>
      ...
      {open && (
        <section className="unified-player-panel">
          <div className="unified-player-tabs">
            ...
            <button type="button" className={tab === 'notifications' ? 'active' : ''} onClick={() => setTab('notifications')}>Notifications {unreadCount > 0 && <span className="unified-mini-count">{unreadCount}</span>}</button>
          </div>
          ...
          {tab === 'online' && (
            <div className="unified-online-pane">
              <div className="unified-menu-header"><strong>Online Players</strong><span>{users.length}</span></div>
