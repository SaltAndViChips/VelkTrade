...
function buyOfferKey(request) { return String(request?.id ?? request?.offerId ?? request?.requestId ?? `${request?.itemId || request?.item_id}-${request?.requesterId || request?.requester_id}`); }
...
function buyRequestSearchText(request) { return [request.id, request.itemId, request.itemTitle, formatPriceDisplay(request.itemPrice || request.offeredIc), request.requesterUsername, request.ownerUsername, request.status, request.createdAt].map(v => vtText(v)).filter(Boolean).join(' ').toLowerCase(); }
...
function normalizeBuyOffer(raw) {
  return {
    ...
    itemTitle: raw?.itemTitle ?? raw?.item_title ?? raw?.title ?? raw?.item?.title,
    itemImage: raw?.itemImage ?? raw?.item_image ?? raw?.image ?? raw?.item?.image,
    itemPrice: raw?.itemPrice ?? raw?.item_price ?? raw?.price ?? raw?.listedPrice ?? raw?.listed_price ?? raw?.item?.price,
...
async function enrichBuyOfferRows(rows) {
  const normalized = safeArray(rows).map(normalizeBuyOffer);
  const enriched = [];
  for (const request of normalized) {
    if ((request.itemImage && request.itemTitle) || !request.itemId) { enriched.push(request); continue; }
    try {
      const data = await api('/api/items/resolve', { method: 'POST', body: JSON.stringify({ itemId: request.itemId, id: request.itemId }) });
      const item = data?.item || data || {};
      enriched.push(normalizeBuyOffer({
        ...request,
        itemTitle: request.itemTitle || item.title || item.itemTitle,
        itemImage: request.itemImage || item.image || item.itemImage,
        itemPrice: request.itemPrice || item.price || item.itemPrice,
        ownerId: request.ownerId || item.userId || item.userid || item.ownerId,
        ownerUsername: request.ownerUsername || item.ownerUsername || item.username
      }));
    } catch { enriched.push(request); }
  }
  return enriched;
}
...
function BuyRequestsTab({ requests = [], currentUser, onRefresh, onLocalStatus }) {
  ...
  async function offerAction(request, actionName) {
    ...
    try {
      ...
      const response = await api(`/api/buy-offers/${encodeURIComponent(request.id)}/${actionName}`, { method: 'POST', body });
      const nextStatus = response?.status || (actionName === 'accept' ? 'accepted' : actionName === 'decline' ? 'declined' : 'countered');
      onLocalStatus?.(request.id, { ...response, status: nextStatus, itemId: request.itemId, itemTitle: response?.itemTitle || request.itemTitle, itemImage: response?.itemImage || request.itemImage, itemPrice: response?.itemPrice || request.itemPrice, offeredIc: response?.offeredIc || request.offeredIc });
      velkToast(`Buy offer ${nextStatus}.`, 'success');
      await onRefresh?.();
    } catch (err) {
  ...
  return <section className="tidy-tab-panel buy-offer-inbox-redesign"><div className="tidy-toolbar buy-offer-toolbar"><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search buy offers by item, user, IC price..." /><div className="segmented-control compact"><button className={scope === 'all' ? 'active' : ''} onClick={() => setScope('all')}>All</button><button className={scope === 'on-my-items' ? 'active' : ''} onClick={() => setScope('on-my-items')}>Inbox</button><button className={scope === 'made-by-me' ? 'active' : ''} onClick={() => setScope('made-by-me')}>Sent</button></div><button onClick={onRefresh}>Refresh</button></div>{error && <p className="error">{error}</p>}<p className="muted tidy-count">Showing {filtered.length} of {normalizedRequests.length} buy offers.</p>{filtered.length === 0 && <p className="muted tidy-empty">No buy offers match this filter/search.</p>}<div className="buy-offer-list-redesign">{filtered.map((request, index) => { const displayPrice = formatPriceDisplay(request.itemPrice); const offeredIc = formatPriceDisplay(request.offeredIc); const title = vtText(request.itemTitle, request.itemId ? `Item #${request.itemId}` : 'Item'); const image = vtText(request.itemImage); const isOnMyItem = Number(request.ownerId) === Number(currentUser.id); const isMadeByMe = Number(request.requesterId) === Number(currentUser.id); const status = vtText(request.status, 'pending'); const actionable = isOnMyItem && ['pending', 'countered'].includes(status); return <article className="buy-offer-card-redesign" key={request.id || index}><header className="buy-offer-card-header"><div><strong>Buy Offer #{vtText(request.id, index + 1)}</strong><small>{vtText(request.createdAt) || 'No timestamp'}</small></div><span className={`status-pill status-${status}`}>{status}</span></header><div className="buy-offer-card-body"><div className="buy-offer-item-preview vt-unified-item-card" data-item-id={request.itemId || ''} data-id={request.itemId || ''} data-title={title} data-price={displayPrice} data-owner-id={request.ownerId || ''} data-owner-username={request.ownerUsername || ''}>{image ? <img src={image} alt={title} /> : <div className="buy-offer-missing-preview"><strong>No preview</strong><span>Item #{request.itemId || 'unknown'}</span></div>}<span className="item-title">{title}</span>{displayPrice && <span className="buy-offer-listing-price">Listed: {displayPrice}</span>}</div><div className="buy-offer-details-panel"><div className="buy-offer-amount-card"><span>Offered IC</span><strong>{offeredIc || 'No IC amount'}</strong></div><div className="buy-offer-meta-grid"><span><strong>Requester</strong>{vtText(request.requesterUsername, 'Unknown')}</span><span><strong>Owner</strong>{vtText(request.ownerUsername, 'Unknown')}</span><span><strong>Direction</strong>{isOnMyItem ? 'Inbox' : isMadeByMe ? 'Sent by you' : 'Visible'}</span>{request.message && <span className="buy-offer-message"><strong>Message</strong>{vtText(request.message)}</span>}</div>{actionable && <div className="tidy-card-actions buy-offer-actions"><button disabled={Boolean(busyId)} onClick={() => offerAction(request, 'accept')}>Accept</button><button disabled={Boolean(busyId)} className="ghost" onClick={() => offerAction(request, 'counter')}>Counter</button><button disabled={Boolean(busyId)} className="danger" onClick={() => offerAction(request, 'decline')}>Decline</button></div>}</div></div></article>; })}</div></section>;
}
...
export default function Trades({ trades = [], buyRequests = [], currentUser, focusedTradeId, onFocusedTradeHandled, onRefresh, onCounter }) {
  ...
  function patchLocalBuyOffer(id, patch) {
    const key = String(id);
    setLocalBuyRequests(current => current.map(request => String(request.id) === key ? normalizeBuyOffer({ ...request, ...patch }) : request));
  }
  ...
  async function loadBuyOfferInbox() {
    ...
    try {
      ...
      setLocalBuyRequests(await enrichBuyOfferRows(rows));
      setBuyRequestsLoaded(true);
      ...
      setError(msg); velkToast(msg, 'error');
