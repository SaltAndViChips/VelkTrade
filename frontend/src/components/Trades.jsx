...
function getIcForUser(trade, userId) {
  return vtText(getTradeMeta(trade).icOffers?.[userId]);
}
...
function buyRequestSearchText(request) {
  return [request.id, request.itemId, request.itemTitle, formatPriceDisplay(request.itemPrice), formatPriceDisplay(request.offeredIc), request.requesterUsername, request.ownerUsername, request.status, request.createdAt, request.message].map(v => vtText(v)).filter(Boolean).join(' ').toLowerCase();
}
...
async function enrichBuyOfferRows(rows) {
  ...
  for (const request of normalized) {
    if ((request.itemImage && request.itemTitle && request.itemPrice) || !request.itemId) {
    ...
    try {
      ...
      enriched.push(normalizeBuyOffer({
        ...request,
        itemTitle: request.itemTitle || item.title || item.itemTitle || item.name,
        itemImage: request.itemImage || item.image || item.itemImage || item.imageUrl,
...
function BuyOfferPreview({ request }) {
  ...
  return (
    <div className="buy-offer-v2-preview" data-item-id={request.itemId || ''} data-id={request.itemId || ''} data-title={title} data-price={listedPrice} data-no-item-popup={!request.itemId ? 'true' : 'false'}>
      <div className="buy-offer-v2-image-box">
        {image ? <img src={image} alt={title} loading="lazy" onError={event => { event.currentTarget.style.display = 'none'; event.currentTarget.parentElement?.classList.add('image-failed'); }} /> : null}
        <div className="buy-offer-v2-fallback">
          <strong>No preview</strong>
          <span>{request.itemId ? `Item #${request.itemId}` : 'Missing item data'}</span>
        </div>
      </div>
      <strong className="buy-offer-v2-item-title">{title}</strong>
      <span className="buy-offer-v2-listed-price">Listed: {listedPrice || 'No listed price'}</span>
    </div>
  );
}
...
function BuyRequestsTab({ requests = [], currentUser, onRefresh, onLocalStatus }) {
  ...
  async function offerAction(request, actionName) {
    ...
    try {
      ...
      onLocalStatus?.(request.id, normalizeBuyOffer({ ...request, ...response, status: nextStatus }));
  ...
  return (
    <section className="buy-offer-v2-shell">
      <div className="buy-offer-v2-toolbar">
        <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search buy offers by item, user, IC price..." />
        <div className="buy-offer-v2-filter-group">
          <button type="button" className={scope === 'all' ? 'active' : ''} onClick={() => setScope('all')}>All</button>
          <button type="button" className={scope === 'on-my-items' ? 'active' : ''} onClick={() => setScope('on-my-items')}>Inbox</button>
          <button type="button" className={scope === 'made-by-me' ? 'active' : ''} onClick={() => setScope('made-by-me')}>Sent</button>
        </div>
      ...
      <div className="buy-offer-v2-list">
        {filtered.map((request, index) => {
          const status = vtText(request.status, 'pending');
          const offeredIc = formatPriceDisplay(request.offeredIc);
          const isOnMyItem = Number(request.ownerId) === Number(currentUser.id);
          const isMadeByMe = Number(request.requesterId) === Number(currentUser.id);
          const actionable = isOnMyItem && ['pending', 'countered'].includes(status);
          const busy = Boolean(busyId);
          return (
            <article className="buy-offer-v2-card" key={request.id || `${request.itemId}-${index}`}>
              <header className="buy-offer-v2-header">
                <div>
                  <strong>Buy Offer #{vtText(request.id, index + 1)}</strong>
                  <small>{request.createdAt ? new Date(request.createdAt).toLocaleString() : 'No timestamp'}</small>
                </div>
                <span className={`status-pill status-${status}`}>{status}</span>
              </header>

              <div className="buy-offer-v2-content">
                <BuyOfferPreview request={request} />

                <div className="buy-offer-v2-details">
                  <div className="buy-offer-v2-offer-box">
                    <span>Offered price</span>
                    <strong>{offeredIc || 'No offered price'}</strong>
                  </div>

                  <div className="buy-offer-v2-meta-grid">
                    <span><strong>Requester</strong>{vtText(request.requesterUsername, 'Unknown')}</span>
                    <span><strong>Owner</strong>{vtText(request.ownerUsername, 'Unknown')}</span>
                    <span><strong>Direction</strong>{isOnMyItem ? 'Inbox' : isMadeByMe ? 'Sent by you' : 'Visible'}</span>
                    <span><strong>Item ID</strong>{vtText(request.itemId, 'Unknown')}</span>
                    {request.message && <span className="buy-offer-v2-message"><strong>Message</strong>{vtText(request.message)}</span>}
                  </div>

                  {actionable && (
                    <div className="buy-offer-v2-actions">
                      <button type="button" disabled={busy} onClick={() => offerAction(request, 'accept')}>Accept</button>
                      <button type="button" disabled={busy} className="ghost" onClick={() => offerAction(request, 'counter')}>Counter</button>
                      <button type="button" disabled={busy} className="danger" onClick={() => offerAction(request, 'decline')}>Decline</button>
                    </div>
                  )}
                </div>
              </div>
...
export default function Trades({ trades = [], buyRequests = [], currentUser, focusedTradeId, onFocusedTradeHandled, onRefresh, onCounter }) {
  ...
  useEffect(() => {
    if (!buyRequestsLoaded && safeArray(buyRequests).length) {
      enrichBuyOfferRows(buyRequests).then(rows => {
        setLocalBuyRequests(rows);
        setBuyRequestsLoaded(true);
      }).catch(() => {});
    }
  }, [buyRequests, buyRequestsLoaded]);
  useEffect(() => {
  ...
  async function action(path) {
    ...
    try {
      ...
      await refreshTradesAndBuyOffers();
      velkToast('Trade updated.', 'success');
    } catch (err) {
      const msg = vtText(err?.message, 'Action failed.');
      setError(msg);
      velkToast(msg, 'error');
    }
  }
  ...
  return (
    <section className="card tidy-trades-page">
      <div className="panel-title-row">
        <div>
          <h2>Trades</h2>
          <p className="muted">Review trade offers, completed trades, counters, and buy offers.</p>
        </div>
        <button onClick={refreshTradesAndBuyOffers}>Refresh</button>
      </div>
      ...
      <div className="segmented-control trades-tabs">
        ...
        <button className={activeTab === 'buy-requests' ? 'active' : ''} onClick={() => setActiveTab('buy-requests')}>Buy Offer Inbox</button>
      </div>

      {activeTab === 'buy-requests' && (
        <>
          {loadingBuyRequests && <p className="muted">Loading buy offers...</p>}
          <BuyRequestsTab requests={displayedBuyRequests} currentUser={currentUser} onRefresh={refreshTradesAndBuyOffers} onLocalStatus={patchLocalBuyOffer} />
        </>
      )}
              ...
              return (
                <article ref={Number(focusedTradeId) === Number(trade.id) ? focusedTradeRef : null} className={`tidy-trade-card ${Number(focusedTradeId) === Number(trade.id) ? 'focused-trade-card' : ''}`} key={id}>
                  <div className="tidy-card-header">
                    <div><strong>Trade #{id}</strong><small>With {otherName(trade)} · Room {vtText(trade.roomId)}</small></div>
                    <span className={`status-pill status-${status}`}>{status}</span>
                  </div>
                  ...
                  <details className="tidy-details">
                    ...
                    <div className="history-chat">{chat.map((message, messageIndex) => <p key={vtText(message?.id, messageIndex)}><strong>{vtText(message?.username, 'User')}:</strong> {vtText(message?.message)}</p>)}</div>
                  </details>
