...
export default function Inventory({ title, items = [], droppableId, readOnly = false, onAddImgurItem, onDoubleClickItem, onClickItem, usernameValue, onUsernameChange, onSearch, folderUsername = '', enableDrag = null }) {
  ...
  const dragEnabled = enableDrag === null ? defaultDragForDroppable(droppableId) : Boolean(enableDrag);
  const { setNodeRef, isOver } = useDroppable({ id: droppableId || 'readonly', data: { zone: droppableId || 'readonly' }, disabled: !dragEnabled });
