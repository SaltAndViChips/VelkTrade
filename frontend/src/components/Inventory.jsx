...
function FolderItemsGrid({ folder, folderItems, selectedIds, toggleSelected, onDoubleClickItem, onClickItem, readOnly }) {
  const color = safeCssColor(folder.color);
  return <div className="inventory-folder-items" style={{ '--folder-color': color }}><div className="item-grid inventory-grid vt-unified-mosaic-grid inventory-folder-item-grid">{folderItems.map(item => readOnly ? <ReadOnlyItem key={item.id || item.image || vtText(item.title)} item={item} onClickItem={onClickItem} /> : <DraggableItem key={item.id || item.image || vtText(item.title)} item={item} onDoubleClickItem={onDoubleClickItem} onClickItem={onClickItem} selectable selected={selectedIds.includes(item.id)} onToggleSelected={toggleSelected} />)}</div></div>;
}
...
function InventoryFolder({ folder, folderItems, open, onToggle }) {
  ...
  return <section className={`inventory-folder-card vt-folder-card ${open ? 'open' : ''}`} data-folder-id={folder.id} data-title={folder.name} style={{ '--folder-color': color }}><button type="button" className="inventory-folder-cover" onClick={onToggle}><div className="inventory-folder-stack">{coverItems.map(item => <img key={item.id || item.image || vtText(item.title)} src={item.image} alt="" aria-hidden="true" />)}<span className="inventory-folder-main-icon">{icon}</span></div><strong className="item-title">{folder.name}</strong></button></section>;
}
