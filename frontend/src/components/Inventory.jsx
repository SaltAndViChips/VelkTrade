...
function safeCssColor(value) { const color = vtText(value).trim(); return /^#[0-9a-f]{3,8}$/i.test(color) || /^rgba?\([\d\s.,%]+\)$/i.test(color) ? color : '#ffdc93'; }
...
function InventoryFolder({ folder, folderItems, selectedIds, toggleSelected, onDoubleClickItem, readOnly = false }) {
  const [open, setOpen] = useState(false);
  const icon = vtText(folder.icon, '📁');
  const color = safeCssColor(folder.color);
  const coverItems = folderItems.slice(0, 4);
  return <section className={`inventory-folder-card vt-folder-card ${open ? 'open' : ''}`} data-folder-id={folder.id} data-title={folder.name} style={{ '--folder-color': color }}><button type="button" className="inventory-folder-cover" onClick={() => setOpen(value => !value)}><div className="inventory-folder-stack">{coverItems.map(item => <img key={item.id || item.image || vtText(item.title)} src={item.image} alt="" aria-hidden="true" />)}<span className="inventory-folder-main-icon">{icon}</span></div><strong className="item-title">{folder.name}</strong></button>{open && <div className="inventory-folder-items"><div className="item-grid inventory-grid vt-unified-mosaic-grid inventory-folder-item-grid">{folderItems.map(item => readOnly ? <ReadOnlyItem key={item.id || item.image || vtText(item.title)} item={item} /> : <DraggableItem key={item.id || item.image || vtText(item.title)} item={item} onDoubleClickItem={onDoubleClickItem} selectable selected={selectedIds.includes(item.id)} onToggleSelected={toggleSelected} />)}</div></div>}</section>;
}

export default function Inventory({ title, items = [], droppableId, readOnly = false, onAddImgurItem, onDoubleClickItem, usernameValue, onUsernameChange, onSearch, folderUsername = '' }) {
  ...
  const visibleItems = shouldLoadFolders ? items.filter(item => !folderItemIds.has(Number(item.id))) : items;
