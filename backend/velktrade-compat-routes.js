...
function installVelkTradeCompatRoutes({ app, authMiddleware, pool, query, run, get }) {
  ...
  async function isAdmin(req) {
    ...
    return Boolean(user?.isAdmin || user?.is_admin || user?.admin || user?.isDeveloper || user?.is_developer || user?.developer || user?.role === 'admin' || user?.role === 'developer' || user?.rank === 'admin' || user?.rank === 'developer' || username === 'salt' || username === 'velkon');
  }
  ...
  async function createAuction(req, res) {
    try {
      ...
      const inserted = await q(`INSERT INTO bazaar_auctions (item_id, seller_id, starting_bid, buyout_price, min_increment, current_bid, status, ends_at, updated_at) VALUES ($1, $2, $3, $4, $5, $3, 'active', NOW() + INTERVAL '365 days', NOW()) RETURNING *`, [item.id, itemOwnerId(item) || userId, startingBid, buyoutPrice, minIncrement]);
      const auction = inserted?.rows?.[0] || { item_id: item.id, seller_id: userId, starting_bid: startingBid, buyout_price: buyoutPrice, min_increment: minIncrement, current_bid: startingBid, status: 'active' };
      res.json({ ok: true, auction: publicAuction({ ...auction, title: item.title, image: item.image, viewer_can_manage: true }, req) });
    } catch (error) { res.status(500).json({ error: error.message || 'Failed to create auction' }); }
  }
