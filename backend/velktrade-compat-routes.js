/* VelkTrade compatibility routes for item popup actions, online toggle, and Bazaar auctions. */
...
function installVelkTradeCompatRoutes({ app, authMiddleware, pool, query, run, get }) {
  ...
  async function ensureAuctionTables() {
    ...
    try { await q("ALTER TABLE bazaar_auctions ALTER COLUMN ends_at SET DEFAULT (NOW() + INTERVAL '365 days')"); } catch {}
  }
  ...
  async function resolveItem(req) {
    ...
    if (image) {
      const item = await one('SELECT * FROM items WHERE image = $1 ORDER BY id DESC LIMIT 1', [image]).catch(() => null);
      if (item) return item;
    }
    if (title) {
      const item = await one('SELECT * FROM items WHERE title = $1 ORDER BY id DESC LIMIT 1', [title]).catch(() => null);
      if (item) return item;
    }
    return null;
  }
  ...
  function isEndedStatus(status) {
    return ['ended', 'completed', 'no_winner', 'bought_out'].includes(String(status || '').toLowerCase());
  }

  async function viewerCanManageAuction(req, auction) {
    return Boolean(await isAdmin(req) || String(currentUserId(req) || '') === String(auction?.seller_id || auction?.sellerId || ''));
  }
  ...
  function publicAuction(row, req) {
    ...
    return {
      ...
      hasBids: Number(row.bid_count || row.bidCount || 0) > 0,
      viewerIsSeller,
  ...
  async function addInterest(req, res) {
    try {
      ...
      await q('INSERT INTO buy_requests (item_id, requester_id, owner_id, created_at) VALUES ($1, $2, $3, NOW()) ON CONFLICT DO NOTHING', [item.id, userId, ownerId]).catch(() => {});
  ...
  async function removeInterest(req, res) {
    try {
      const userId = currentUserId(req);
      const item = await resolveItem(req);
      if (item?.id && userId) await q('DELETE FROM buy_requests WHERE item_id = $1 AND requester_id = $2', [item.id, userId]).catch(() => {});
  ...
  async function getInterest(req, res) {
    try {
      ...
      const rows = await many('SELECT u.id, u.username, u.is_verified FROM buy_requests br JOIN users u ON u.id = br.requester_id WHERE br.item_id = $1', [item.id]).catch(() => []);
      res.json({ users: rows });
    } catch {
      res.json({ users: [] });
    }
  ...
  async function removeItem(req, res) {
    try {
      ...
      await q('UPDATE items SET price = NULL, show_bazaar = FALSE, trade_pending = TRUE WHERE id = $1', [item.id]).catch(() => {});
    ...
    try {
      ...
      const rows = await many(`
        ...
        WHERE i.userid = $1
          ...
          AND NOT EXISTS (SELECT 1 FROM bazaar_auctions a WHERE a.item_id = i.id AND a.status = 'active')
  ...
  async function loadAuctionRows(req, status) {
    ...
    const rows = await many(`
      SELECT a.*, i.title, i.image, u.username AS seller_username, COALESCE(u.is_verified, false) AS seller_verified,
             wu.username AS winner_username, COUNT(b.id) AS bid_count
      ...
      ${where}
      GROUP BY a.id, i.title, i.image, u.username, u.is_verified, wu.username
  ...
  async function listAuctions(req, res) {
    try {
      ...
      const auctions = await loadAuctionRows(req, status);
  ...
  async function listAuctionBids(req, res) {
    try {
      ...
      const rows = await many(`
        SELECT b.id, b.amount, b.created_at, u.id AS bidder_id, u.username AS bidder_username, COALESCE(u.is_verified, false) AS bidder_verified
        FROM bazaar_auction_bids b
      ...
      res.status(500).json({ error: error.message || 'Failed to load bids' });
    }
  }
  ...
  async function createAuction(req, res) {
    try {
      ...
      const inserted = await q(`
        ...
        VALUES ($1, $2, $3, $4, $3, 'active', NOW() + INTERVAL '365 days', NOW()) RETURNING *
      `, [item.id, itemOwnerId(item) || userId, startingBid, buyoutPrice]);
      const auction = inserted?.rows?.[0] || { item_id: item.id, seller_id: userId, starting_bid: startingBid, buyout_price: buyoutPrice, current_bid: startingBid, status: 'active' };
      res.json({ ok: true, auction: publicAuction({ ...auction, title: item.title, image: item.image, viewer_can_manage: true }, req) });
    } catch (error) {
  ...
  async function endAuction(req, res) {
    try {
      ...
      const auction = await one('SELECT * FROM bazaar_auctions WHERE id = $1', [auctionId]).catch(() => null);
      if (!auction) return res.status(404).json({ error: 'Auction not found' });
      if (!(await viewerCanManageAuction(req, auction))) return res.status(403).json({ error: 'Not allowed' });
      const winnerId = req.body?.winnerId || req.body?.winner_id || null;
      if (winnerId) {
        const bid = await one('SELECT * FROM bazaar_auction_bids WHERE auction_id = $1 AND bidder_id = $2 ORDER BY amount DESC LIMIT 1', [auctionId, winnerId]).catch(() => null);
        if (!bid) return res.status(400).json({ error: 'Winner must be one of the bidders' });
        await q("UPDATE bazaar_auctions SET status = 'completed', winner_id = $1, current_bid = $2, updated_at = NOW() WHERE id = $3", [winnerId, Number(bid.amount), auctionId]);
        await q('UPDATE items SET trade_pending = TRUE WHERE id = $1', [auction.item_id]).catch(() => {});
        return res.json({ ok: true, auctionId: Number(auctionId), status: 'completed', winnerId, currentBid: Number(bid.amount) });
      }
      await q("UPDATE bazaar_auctions SET status = 'no_winner', winner_id = NULL, updated_at = NOW() WHERE id = $1", [auctionId]);
      res.json({ ok: true, auctionId: Number(auctionId), status: 'no_winner', winnerId: null });
    } catch (error) {
  ...
  async function deleteAuction(req, res) {
    try {
      ...
      if (!auction) return res.status(404).json({ error: 'Auction not found' });
      if (!(await viewerCanManageAuction(req, auction))) return res.status(403).json({ error: 'Not allowed' });
      await q('DELETE FROM bazaar_auctions WHERE id = $1', [auctionId]);
      res.json({ ok: true, auctionId: Number(auctionId), deleted: true });
    } catch (error) {
  ...
  app.get('/api/bazaar/auctions/:auctionId/bids', auth, listAuctionBids);
  app.post('/api/bazaar/auctions/:auctionId/bid', auth, placeBid);
  app.post('/api/bazaar/auctions/:auctionId/buyout', auth, buyoutAuction);
  app.post('/api/bazaar/auctions/:auctionId/end', auth, endAuction);
  app.delete('/api/bazaar/auctions/:auctionId', auth, deleteAuction);
}
