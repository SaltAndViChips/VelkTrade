...
function allowedOrigin(origin) {
  if (!origin) return '';
  const configured = [process.env.FRONTEND_ORIGIN, process.env.PUBLIC_FRONTEND_URL, process.env.CORS_ORIGIN].filter(Boolean).map(value => String(value).replace(/\/$/, ''));
...
function isIntegerId(value) { return /^\d+$/.test(String(value || '').trim()); }
...
function isAdminOrDeveloper(req) {
  ...
  return Boolean(req.user?.isAdmin || req.user?.is_admin || req.user?.admin || req.user?.isDeveloper || req.user?.is_developer || req.user?.developer || req.user?.role === 'admin' || req.user?.role === 'developer' || username === 'salt' || username === 'velkon');
}
...
function shouldOverride(path) { return typeof path === 'string' && TARGET_PATHS.has(path); }
