...
function installItemFolderNoteRoutes({ app, authMiddleware }) {
  ...
  async function loadItem(itemId) {
    return get('SELECT * FROM items WHERE id = ?', [Number(itemId)]);
  }
  ...
  async function requireOwnedItem(req, res, itemId, allowAdmin = false) {
    ...
    if (!ownsItem(req, item) && !(allowAdmin && isAdminOrDeveloper(req))) {
  ...
  async function listFolders(req, res) {
    try {
      ...
      console.error('list item folders failed:', error);
      res.status(500).json({ error: error.message || 'Failed to load folders' });
    }
  }
  ...
  async function createFolder(req, res) {
    try {
      ...
      const result = await run(`INSERT INTO item_folders (user_id, name, color, updated_at) VALUES (?, ?, ?, NOW()) RETURNING id`, [uid, name, color]);
      const id = result.rows?.[0]?.id || result.lastID;
      await audit(req, 'item_folder.created', 'item_folder', id, { name, color });
      res.json({ ok: true, folder: { id, name, color, itemCount: 0 } });
    } catch (error) {
      console.error('create item folder failed:', error);
  ...
  async function deleteFolder(req, res) {
    try {
      ...
      console.error('delete item folder failed:', error);
  ...
  async function getItemOrganization(req, res) {
    try {
      ...
      const folders = await all(`
        SELECT f.id, f.name, f.color, a.created_at AS "assignedAt"
        FROM item_folder_assignments a
        JOIN item_folders f ON f.id = a.folder_id
        WHERE a.user_id = ? AND a.item_id = ?
        ORDER BY f.name ASC
      `, [uid, Number(itemId)]).catch(() => []);
      res.json({ ok: true, itemId: Number(itemId), note: noteRow?.note || '', noteUpdatedAt: noteRow?.updatedAt || null, folders });
    } catch (error) {
      console.error('get item organization failed:', error);
      res.status(500).json({ error: error.message || 'Failed to load item organization' });
    }
  }
  ...
  async function saveItemNote(req, res) {
    try {
      ...
      const itemId = req.params.itemId || req.params.id;
      if (!uid) return res.status(401).json({ error: 'Not authenticated' });
      const item = await requireOwnedItem(req, res, itemId, false);
      if (!item) return;
      const note = String(req.body?.note || '').slice(0, 5000);
      await run(`
        INSERT INTO item_private_notes (user_id, item_id, note, updated_at)
        VALUES (?, ?, ?, NOW())
        ON CONFLICT (user_id, item_id)
        DO UPDATE SET note = EXCLUDED.note, updated_at = NOW()
      `, [uid, Number(itemId), note]);
      await audit(req, 'item_note.saved', 'item', itemId, { noteLength: note.length });
      res.json({ ok: true, itemId: Number(itemId), note });
    } catch (error) {
      console.error('save item note failed:', error);
  ...
  async function assignFolder(req, res) {
    try {
      ...
      await run(`
        INSERT INTO item_folder_assignments (user_id, item_id, folder_id)
        VALUES (?, ?, ?)
        ON CONFLICT DO NOTHING
      `, [uid, Number(itemId), Number(folderId)]);
      ...
      console.error('assign item folder failed:', error);
      res.status(500).json({ error: error.message || 'Failed to assign folder' });
    }
  }
  ...
  async function unassignFolder(req, res) {
    try {
      ...
      const uid = userId(req);
      const itemId = req.params.itemId || req.params.id;
      const folderId = req.params.folderId || req.body?.folderId || req.body?.folder_id;
      if (!uid) return res.status(401).json({ error: 'Not authenticated' });
      if (!validId(folderId)) return res.status(400).json({ error: 'Invalid folder id' });
      const item = await requireOwnedItem(req, res, itemId, false);
      if (!item) return;
      ...
      console.error('unassign item folder failed:', error);
      res.status(500).json({ error: error.message || 'Failed to remove folder assignment' });
    }
  }
