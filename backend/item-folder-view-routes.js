...
function installItemFolderViewRoutes({ app, authMiddleware }) {
  ...
  async function resolveTargetUserId(req) {
    const username = String(req.params.username || req.query.username || '').trim();
    if (username) {
      const user = await get(`SELECT id FROM users WHERE LOWER(username) = LOWER(?) LIMIT 1`, [username]).catch(() => null);
      return Number(user?.id || 0);
    }
    return requesterId(req);
  }
  ...
  async function foldersWithItems(req, res) {
    try {
      ...
      res.json({ ok: true, userId: uid, folders });
    } catch (error) {
  ...
  app.get('/api/item-folders/with-items', auth, foldersWithItems);
  app.get('/api/inventory/folders/with-items', auth, foldersWithItems);
  app.get('/api/item-folders-with-items/:username', auth, foldersWithItems);
  app.get('/api/inventory/:username/folders-with-items', auth, foldersWithItems);
  app.get('/api/profile/:username/folders-with-items', auth, foldersWithItems);
}
