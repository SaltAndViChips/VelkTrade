    ...
    try {
      ...
      const rows = await all(`
        SELECT
          ...
          COALESCE(COUNT(a.item_id), 0) AS "itemCount"
        FROM item_folders f
        LEFT JOIN item_folder_assignments a ON a.folder_id = f.id AND a.user_id = f.user_id
        WHERE f.user_id = ?
        GROUP BY f.id
      ...
      const assignments = await all(`
        SELECT a.folder_id AS "folderId", a.item_id AS "itemId"
        FROM item_folder_assignments a
        JOIN item_folders f ON f.id = a.folder_id AND f.user_id = a.user_id
      ...
      const folders = rows.map(folder => ({
        ...folder,
        itemIds: byFolder.get(Number(folder.id)) || []
      }));
