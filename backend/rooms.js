...
function joinRoom(roomId, player) {
  ...
  if (room.players.length >= 2 && !room.players.some(p => p.id === player.id)) throw new Error('Room is full');
...
function assertPlayerInRoom(room, userId) {
  if (!room.players.some(player => player.id === userId)) throw new Error('You are not in this room');
}
...
function everyoneAccepted(room) {
  return room.players.length === 2 && room.players.every(p => room.accepted[p.id]);
}
...
async function finalizeTrade(room) {
  ...
  try {
    ...
    for (const itemId of playerAItems) await run('UPDATE items SET userId = ? WHERE id = ?', [playerB.id, itemId]);
    for (const itemId of playerBItems) await run('UPDATE items SET userId = ? WHERE id = ?', [playerA.id, itemId]);
...
async function leaveRoom(roomId, userId) {
  ...
  if (!room) throw new Error('Room not found');
  assertPlayerInRoom(room, userId);
  if (!room.completed && room.players.length === 2) await saveTradeSnapshot(room, 'declined');
  rooms.delete(roomId);
  return room;
}
